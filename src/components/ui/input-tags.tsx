...
const InputTags = ({
  ...
  >
    {tags.map(t => (
      <span
        key={t}
        className="inline-flex items-center gap-1.5 rounded-md bg-zinc-600/10 px-2 py-1 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-600/20 focus:ring-2 focus:ring-ring focus:ring-offset-1 focus:outline-none"
        tabIndex={0}
        role="button"
    ...
    <input
      className={cn(
        'ml-1 h-8 min-w-0 flex-1 appearance-none border-0 bg-transparent px-2 py-1 text-sm font-medium text-zinc-700 ring-0 transition-all duration-200 ease-out outline-none placeholder:text-zinc-500 placeholder:capitalize focus:outline-none dark:text-white',
